...
exports.handleMessage = (client, msg) => {
  // Ignore Bots if True
  if (client.config.ignoreBots && msg.author.bot && (msg.author.id !== client.user.id)) return false;
  // Ignore Self if true
  if (client.config.ignoreSelf && msg.author.id === client.user.id && (msg.author.id !== client.user.id)) return false;
  // Ignore other users if selfbot
  if (!client.user.bot && msg.author.id !== client.user.id && (msg.author.id !== client.user.id)) return false;
  // Ignore self if bot
  if (client.user.bot && msg.author.id === client.user.id && (msg.author.id !== client.user.id)) return false;
  return true;
};
